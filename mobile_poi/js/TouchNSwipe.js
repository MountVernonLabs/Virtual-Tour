...
(function(window, $)
{
	...
	function ImageSlider(slider, _slides, varsObj)
	{
		...
		var _vars = $.extend({ minDragDistance:100, animDuration:0.3, maxZoom:5, minZoom:1, scaleMode:"proportionalInside", ease:Power4.easeOut, loop:true, allowZoom:true, allowDrag:true, allowSwipe:true, doubleTapZoom:2, zoomStep:0.5, dragStep:10, adjustWidth:0, adjustHeight:0, playDuration:5, autoPlay:false, loadIndexOnly:true, preloaderUrl:"assets/preloader.gif", resizeDuration:-1, allowKeys:true }, varsObj),
			...
			prevSlide = $("<div style='width:100%; height:100%; left:-100%; top:-1px; position:absolute; border: " + slideBorderColor + "; overflow:hidden'></div>"),
			curSlide =  $("<div style='width:100%; height:100%; left:-1px; top:-1px; position:absolute; border: " + slideBorderColor + "; overflow:hidden'></div>"),
			nextSlide = $("<div style='width:100%; height:100%; left:100%; top:-1px; position:absolute; border: " + slideBorderColor + "; overflow:hidden'></div>"),
		...
		function resetImageAt(i)
		{
			if(i >= 0 && i < _slides.length && _slides[i].loaded)
			{
				...
				if(_vars.scaleMode == "proportionalOutside")
				{
...
(function(window, $)
{
	...
	{
		var _vars = $.extend( { initShow:true, setHolderVisibility:true, animDuration:0.25, visibilityDelay:0.1, showCssObj:{ autoAlpha:1 }, hideCssObj:{ autoAlpha:0 } }, varsObj),
			isShow = _vars.initShow,
		...
		function _show(value, duration)
		{
			...
			if(value !== undefined)
			{
				...
				if(isShow)
				{
					if(setHolderVisibility)
					{
						tl.add(TweenMax.to(sliderDiv.parent(), parentDuration, { display:"block", overwrite:"all" }));
					}
				...
				else
				{
					...
					if(setHolderVisibility)
					{
						tl.add(TweenMax.to(sliderDiv.parent(), parentDuration, { display:"none", overwrite:"all", immediateRender:false }));
					}
				}
	...
	TouchNSwipe.init = function()
	{
		...
		for(i = 0; i < sliderHolderLen; i++)
		{
			...
			var tnsVars = Utils.getAttrObjectFromString(sliderHolder.data("options"));
